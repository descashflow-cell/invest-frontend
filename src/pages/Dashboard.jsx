...
export default function Dashboard() {
  ...
  const refresh = useCallback(async (m) => {
    setLoading(true);
    try { setData(await getSummary(m)); }
  ...
  const toggleNav = () => {
    setNavCollapsed((v) => {
      const next = !v;
      try { localStorage.setItem("navCollapsed", next ? "1" : "0"); } catch (e) { /* ignore */ }
      return next;
    });
  };
  ...
  return (
    <div className="relative z-10 min-h-screen">
      <SideNav active={active} onChange={setActive} collapsed={navCollapsed} onToggleCollapsed={toggleNav} />

      <main className={`${navCollapsed ? "lg:ml-[72px]" : "lg:ml-[240px]"} transition-[margin] duration-300 ease-out px-4 sm:px-8 lg:px-12 py-8 sm:py-12 max-w-[1400px]`}>
        ...
        <footer className="mt-16 pt-8 border-t border-white/5 flex items-center justify-between text-xs text-neutral-600">
          ...
          <span className="uppercase tracking-[0.2em]">Tactical Minimalism</span>
        </footer>
      </main>
    </div>
  );
